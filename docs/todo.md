...
- [x] 数字ボタン（0-9）のクリックで数値を入力できるようにする
- [x] 初期状態（"0"）の時は、数字を押すと置き換える（"0" → "1"）
- [x] 既に数値が入力されている場合は、末尾に追加する（"12" → "123"）
- [x] 状態管理：現在の入力値を保持する state を適切に更新する

### 1.2 表示の更新

- [x] 入力された数値が Display コンポーネントに正しく表示されることを確認
...
- [x] 現在選択されている演算子（+, -, ×, ÷）を保持する state を追加
- [x] 演算子ボタンをクリックした時に、選択された演算子を保存する
- [x] 演算子が選択された時に、現在の入力値を「前の値」として保存する

### 2.2 演算子の切り替え

- [x] 既に演算子が選択されている状態で別の演算子を押した場合、演算子を切り替える
- [x] 演算子を切り替えた後、新しい数値の入力待ち状態にする => 演算子を押した後に、display を 0 にして、prevNumber に現在の number を移し替える

## フェーズ 3: 等号による計算実行

### 3.1 計算ロジックの実装

- [x] 四則演算（+, -, ×, ÷）それぞれの計算処理を実装（計算関数を作成）
- [x] 等号ボタンをクリックした時に、前の値と現在の値、演算子を使って計算を実行
- [x] 計算結果を Display に表示する
- [x] 計算後、結果を「前の値」として保持し、連続計算に備える

### 3.2 連続計算の対応

- [x] 計算結果が表示された状態で、演算子を押すと連続計算が可能になる
- [x] 計算結果が表示された状態で、数字を押すと新しい計算を開始する

## フェーズ 4: クリア機能の実装

### 4.1 C（Clear）ボタン

- [x] C ボタンをクリックした時に、現在の入力値を"0"にリセット
- [x] 演算子や前の値は保持したまま、現在の表示のみをクリア

### 4.2 AC（All Clear）ボタン

- [x] AC ボタンをクリックした時に、すべての状態を初期化
- [x] 表示値、演算子、前の値、計算履歴などをすべてリセット
